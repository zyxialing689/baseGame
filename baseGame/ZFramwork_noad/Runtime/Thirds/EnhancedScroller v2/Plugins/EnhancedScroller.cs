...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler,IPointerDownHandler,IPointerUpHandler
    {
        ...
        public EnhancedScrollerCellView GetActiveCenterCellView()
        {
            float curDis = float.MaxValue;
            float tempDis;
            EnhancedScrollerCellView minDisCell = null;
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
                tempDis = Vector2.Distance(transform.position, _activeCellViews[i].transform.position);
                if (tempDis < curDis)
                {
                    curDis = tempDis;
                    minDisCell = _activeCellViews[i];
                }
            }
            return minDisCell;
        }
        ...
        /// to call ReloadData for that to work.
        /// </summary>
        public void SpeRefreshActiveCellViews()
        {
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
                _activeCellViews[i].SpeRefreshCellView();
            }
        }
        /// <summary>
        /// This calls the RefreshCellView method on each active cell.
        ...
        /// to call ReloadData for that to work.
        /// </summary>
        public void SpeRefreshActiveCellViews2()
        {
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
                _activeCellViews[i].SpeRefreshCellView2();
            }
        }

        public List<T> GetActiveCellViews<T>()where T:EnhancedScrollerCellView
        {
            List<T> list = new List<T>();
            for (int i = 0; i < _activeCellViews.Count; i++)
            {
                list.Add(_activeCellViews[i] as T);
            }
            return list;
        }
